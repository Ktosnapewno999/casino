...
function publicUser(user) {
  return { username: user.username, email: user.email, verified: Boolean(user.verified) };
}
...
app.post("/api/register", async (req, res) => {
  ...
  db.prepare(`
    INSERT INTO users (username, email, password_salt, password_hash, verification_hash, verification_expires_at)
    VALUES (?, ?, ?, ?, ?, ?)
  `).run(username, email, salt, hash, verificationHash, expiresAt);
...
app.post("/api/verify-email", (req, res) => {
  ...
  if (user.verified) return res.json({ ok: true, user: publicUser(user) });
  ...
  res.json({ ok: true, user: { username: user.username, email: user.email, verified: true } });
});
